...
export default function Register() {
    ...
    useEffect(() => {
        ...
        if (ref) {
            console.log('🔗 Código de indicação detectado:', ref);
            setReferralCode(ref);
        }
    }, [searchParams]);
    ...
    const handleRegister = async (e) => {
        ...
        if (password !== confirmPassword) {
            return setError(t('auth.error_passwords_mismatch'));
        }
        ...
        try {
            ...
            if (!response.ok) throw new Error(data.error || t('auth.error_register'));
        ...
        <div className="w-full max-w-[480px] animate-fadeInUp">
            ...
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-white dark:border-slate-800 rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] p-8 md:p-12 relative overflow-hidden">
                ...
                <div className="text-center mb-10 relative z-10">
                    ...
                    <h2 className="text-4xl font-display font-black text-slate-900 dark:text-white tracking-tight mb-3">{t('auth.register_title')}</h2>
                ...
                <form onSubmit={handleRegister} className="space-y-5 relative z-10">
                    ...
                    <div className="space-y-2">
                        <label className="block text-[11px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest ml-1">{t('auth.name_label')}</label>
                        <input
                            type="text"
                            value={name}
                    ...
                    <div className="space-y-2">
                        <label className="block text-[11px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest ml-1">{t('auth.email_label')}</label>
                        <input
                            type="email"
                            value={email}
                            onChange={e => setEmail(e.target.value)}
                            className="w-full px-6 py-4 rounded-2xl bg-slate-100/50 dark:bg-slate-950/50 border-2 border-transparent focus:bg-white dark:focus:bg-slate-900 focus:border-indigo-500 outline-none transition-all text-slate-800 dark:text-slate-100 font-bold placeholder:text-slate-400 dark:placeholder:text-slate-600"
                            placeholder={t('auth.email_register_placeholder')}
                            required
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <label className="block text-[11px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest ml-1">{t('auth.password_create_label')}</label>
                            <input
                                type="password"
                                value={password}
                        ...
                        <div className="space-y-2">
                            <label className="block text-[11px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest ml-1">{t('auth.password_confirm_label')}</label>
                            <input
                                type="password"
                ...
                <div className="mt-12 text-center relative z-10">
                    <p className="text-slate-500 dark:text-slate-500 font-bold text-sm">
                        {t('auth.already_have_account')} <Link to="/login" className="text-indigo-600 dark:text-indigo-400 hover:underline underline-offset-4">{t('auth.login_link')}</Link>
                    </p>
                </div>
            </div>
        </div>
