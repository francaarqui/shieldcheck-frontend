...
export default function Analyze() {
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            setExtractedSource('Imagem (print)');
            ...
            if (visionRes.ok) {
                ...
                setResult({
                    ...
                    status: visionData.isScam ? t('tools.analyze.pdf.verdict') + ': GOLPE' : t('tools.analyze.pdf.verdict') + ': SEGURO',
        ...
        <div className="animate-slide-up max-w-5xl mx-auto space-y-12 pb-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div className="flex-1">
                    <h2 className="text-4xl font-display font-black text-slate-900 dark:text-white tracking-tight" dangerouslySetInnerHTML={{ __html: t('tools.analyze.title') }}></h2>
                    <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg font-medium">{t('tools.analyze.subtitle')}</p>
                </div>
            </div>
                        ...
                        <div className="space-y-6 relative z-10">
                            ...
                            <div className="relative group">
                                ...
                                <textarea
                                    ...
                                    className="relative w-full h-56 p-8 bg-slate-50/30 dark:bg-slate-950/30 backdrop-blur-sm border-2 border-slate-100 dark:border-slate-800 rounded-3xl focus:ring-0 focus:border-indigo-500 outline-none transition-all text-slate-800 dark:text-slate-100 font-medium text-lg placeholder:text-slate-400 custom-scrollbar resize-none"
                                />
                                <div className="absolute bottom-6 right-8 flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest pointer-events-none">
                                    <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                    {t('tools.analyze.input_monitor')}
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                                <div className="lg:col-span-5 grid grid-cols-3 gap-3">
                                    ...
                                    <button onClick={() => fileInputRef.current.click()} className="flex flex-col items-center justify-center gap-2 p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/20 transition-all group shadow-sm">
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">{t('tools.analyze.btn_image')}</span>
                                    </button>

                                    <button onClick={() => audioInputRef.current.click()} className="flex flex-col items-center justify-center gap-2 p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/20 transition-all group shadow-sm">
                                        <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-indigo-600 transition-colors">
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">{t('tools.analyze.btn_audio')}</span>
                                    </button>

                                    <button onClick={() => videoInputRef.current.click()} className="flex flex-col items-center justify-center gap-2 p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/20 transition-all group shadow-sm">
                                        <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-indigo-600 transition-colors">
                                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 00-2 2z" /></svg>
                                        </div>
                                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">{t('tools.analyze.btn_video')}</span>
                                    </button>
                                </div>
                                ...
                                <button
                                    ...
                                    className="lg:col-span-7 h-auto bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black rounded-2xl hover:opacity-90 transition-all shadow-xl shadow-slate-200 dark:shadow-none disabled:opacity-50 flex items-center justify-center gap-4 text-lg py-4 group overflow-hidden relative"
                                >
                                    ...
                                    {isLoading || isProcessingMedia ? (
                                        <><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-current"></div> {processingStatus || 'Analisando...'}</>
                                    ) : (
                                        <>
                        ...
                        <div className="glass-card rounded-[3rem] border border-white dark:border-slate-800 shadow-2xl overflow-hidden relative">
                            ...
                            <div className="p-10 flex flex-col lg:flex-row gap-12">
                                ...
                                <div className="flex flex-col items-center justify-center space-y-4">
                                    ...
                                    `}>
                                        <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">{t('tools.analyze.risk_score')}</p>
                                ...
                                <div className="flex-1 space-y-10">
                                    <div className="space-y-4">
                                        <h4 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('tools.analyze.detected_signals')}</h4>
                                    ...
                                    {result.voiceIdentity && (
                                        <div className="p-8 bg-slate-900 text-white rounded-[2rem] border border-indigo-500/30 relative overflow-hidden transition-all hover:scale-[1.02]">
                                            <div className="absolute top-0 right-0 p-4 opacity-10">
                                                <svg className="w-20 h-20 text-indigo-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 10.99h7c-.53 4.12-3.28 7.79-7 8.94V12H5V6.3l7-3.11v8.8z" /></svg>
                                            </div>
                                            <h4 className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-3">ShieldCheck Voice Identity</h4>
                                            <div className="flex items-center gap-4 mb-4">
                                                <div className={`w-3 h-3 rounded-full ${result.voiceIdentity.isSynthetic ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}></div>
                                                <p className="text-lg font-black truncate">
                                                    {result.voiceIdentity.isSynthetic ? 'VOZ SINTÉTICA (DEEPFAKE)' : 'VOZ HUMANA AUTÊNTICA'}
                                                </p>
                                                <span className="bg-white/10 px-2 py-1 rounded text-[10px] font-bold">Confiança: {result.voiceIdentity.voiceConfidence}%</span>
                                            </div>
                                            <div className="space-y-1">
                                                {result.voiceIdentity.acousticSignals.map((signal, idx) => (
                                                    <p key={idx} className="text-xs text-slate-400 flex items-center gap-2">
                                                        <span className="w-1 h-1 bg-indigo-400 rounded-full"></span> {signal}
                                                    </p>
                                                ))}
                                            </div>
                                        </div>
                                    ...
                                    <div className="p-8 bg-indigo-50 dark:bg-indigo-900/30 rounded-[2rem] border border-indigo-100 dark:border-indigo-900/30 relative overflow-hidden">
                                        ...
                                        <h4 className="text-xs font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest mb-3">{t('tools.analyze.recommendation')}</h4>
